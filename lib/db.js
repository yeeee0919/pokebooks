...
export async function ensureSchema() {
  ...
  await db`CREATE TABLE IF NOT EXISTS cardmarket_snapshots (
  ...
  await db`CREATE INDEX IF NOT EXISTS cardmarket_snapshots_scraped_at_idx
    ON cardmarket_snapshots (scraped_at DESC)`;
}
...
function cardmarketMeta(row) {
  return {
    date: row.scrape_date,
    seller: row.seller || null,
    scraped_at: isoTs(row.scraped_at),
  };
}

export async function listCardmarketDates(limit = 90) {
  const n = Math.min(Math.max(Number(limit) || 90, 1), 366);
  const rows = await sql()`
    SELECT scrape_date, seller, scraped_at
    FROM cardmarket_snapshots
    ORDER BY scrape_date DESC
    LIMIT ${n}
  `;
  return rows.map(cardmarketMeta);
}

export async function getCardmarketSnapshot(date) {
  const rows = date
    ? await sql()`
        SELECT scrape_date, seller, scraped_at, payload, updated_at
        FROM cardmarket_snapshots
  ...
  return {
    ...cardmarketMeta(row),
    updated_at: isoTs(row.updated_at),
    snapshot: payload,
  };
}

export async function upsertCardmarketSnapshot({ date, seller, scrapedAt, payload }) {
  const db = sql();
  const ts = scrapedAt ? new Date(scrapedAt) : null;
  const scraped = ts && !Number.isNaN(ts.getTime()) ? ts : null;
  await db`
    INSERT INTO cardmarket_snapshots (scrape_date, seller, scraped_at, payload, updated_at)
    VALUES (${date}, ${seller}, ${scraped}, ${db.json(payload)}, now())
    ON CONFLICT (scrape_date) DO UPDATE SET
      ...
      updated_at = now()
  `;
  return getCardmarketSnapshot(date);
}
