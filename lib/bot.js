...
import {
  ...
  listPhotosForDraft, tryClaimAlbumExtract, countPhotosForDraft, syncDraftPhotoList,
  tryConsumeTelegramUpdate, attachPhotoIfNew, deletePhotosForDraft,
} from './db.js';
...
export async function handleUpdate(update) {
  if (!(await tryConsumeTelegramUpdate(update.update_id))) return;
  ...
  if (fileId) {
    ...
    if (file) {
      ...
      const attached = await attachPhotoIfNew({
        id: photoId,
        draftId: draft.id,
        mime: file.mime,
        bytes: file.bytes,
      ...
      if (attached.added) {
        ...
        draft.photos.push({ id: attached.id, mime: file.mime });
...
async function refreshGuessesFromAllPhotos(draft, products) {
  ...
  // Prefer payment-screen total when OCR saw order + payment (still needs user 「對」 at amount step)
  if (guesses.amountEur != null && (draft.fields.currency || guesses.currency || 'EUR') === 'EUR') {
    if (draft.guesses.originalAmount == null) draft.guesses.originalAmount = guesses.amountEur;
  }
...
async function cmdCancel(chatId) {
  ...
  await deletePhotosForDraft(draft.id);
