...
import { searchProducts, parseKind, parseScope, parseCurrencyAmount, looksLikeInstruction } from './parse.js';
import {
...
export async function handleUpdate(update) {
  ...
  // Same-order instruction mid-flight
  if (text && looksLikeInstruction(text)) {
    draft.seedText = [draft.seedText, text].filter(Boolean).join('\n');
    await saveDraft(draft);
    await sendMessage(chatId, `了解，${(draft.photos || []).length || 1} 張都算同一筆。繼續回答下面的問題即可。`);
    const q = nextQuestion(draft, settings);
    draft.waitingFor = q.waitingFor;
    await saveDraft(draft);
    await sendMessage(chatId, q.text);
    return;
  }
...
function helpText() {
  return [
    ...
    '多張收據／明細：用相簿一次傳，或連續傳——都算同一單。',
    '確認時可直接改：台幣、私人、商家無、日期今天…',
    '/later 擱成待補',
    '/cancel 丟掉這一筆',
    '/pending 列出待補',
    '/open 前八碼 繼續待補',
