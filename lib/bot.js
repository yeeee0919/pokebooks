...
import {
  ensureSchema, getOrCreateActiveDraft, getActiveDraftForChat, saveDraft, listDrafts, getDraft,
  getLedger, putLedger, getBind, setBind, getPairing, clearPairing,
  savePhoto, listPhotosForDraft, tryClaimAlbumExtract,
} from './db.js';
import { sendMessage, downloadTelegramFile, ocrPhoto, isAllowedUser } from './telegram.js';

const ALBUM_SETTLE_MS = 3000;
...
export async function handleUpdate(update) {
  ...
  // Wait for siblings (album or rapid sequential sends), then OCR once
  if (addedPhoto) {
    const draftId = draft.id;
    const countAtSave = (draft.photos || []).length;
    await sleep(ALBUM_SETTLE_MS);
    let latest = await getDraft(draftId);
    if (!latest || latest.status !== 'active') return;
    const total = (latest.photos || []).length;
    if (total > countAtSave) return; // another photo still settling
    if ((latest.lastExtractPhotoCount || 0) >= total) return;
    const claimed = await tryClaimAlbumExtract(latest.id, total);
    if (!claimed) return;
    try {
    ...
    const label = total > 1 ? `${total} 張圖` : '1 張圖';
    await sendMessage(chatId, `已收到 ${label}，當作同一單一起看過。\n\n${q.text}`);
    return;
  }
