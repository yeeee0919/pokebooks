...
The previous app stored the live ledger in `localStorage`. A Telegram webhook cannot write that store, so drafts and posted rows live in one EU Postgres database. The web UI and the bot are both clients. After a one-time import, the browser is a cache plus editor, not the source of truth.
